...
class QJobMapper:
    ...
    def __call__(self, func, population):
        ...
        """
        qjobs_ = []
        logger.debug(f"Updating params for QJob {qjob}...")
...
class QPUCircuitMapper:
    ...
    def __init__(self, qpus: list[QPU], circuit: Union[dict, QuantumCircuit, CunqaCircuit], **run_parameters: Any):
        """
        Class constructor.
        ...
        if isinstance(circuit, QuantumCircuit):
            ...
            raise TypeError(f"Parametric circuit must be <class 'qiskit.circuit.quantumcircuit.QuantumCircuit'>, but {type(circuit)} was provided [{TypeError.__name__}].")
    ...
    def __call__(self, func, population):
        ...
        try:
            ...
            for i, params in enumerate(population):
                ...
                qjobs.append(run(qpu, circuit_assembled, **self.run_parameters))
            ...
            raise RuntimeError(f"Error while assigning parameters to Qiskit's QuantumCircuit, "
                               f"please check they have the correct size [{type(error).__name__}]: {error}.")