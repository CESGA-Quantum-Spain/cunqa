    ...
    def density_matrix(self) -> Union[dict[np.array], np.array]:
        try:
            if ("results" in list(self._result.keys()) 
                ...
                density_matrix = {} # All of this is because we can store multiple density matrices with labels different from "density_matrix"
                for k, v in self._result["results"][0]["metadata"]["result_types"].items():
                ...
                if len(density_matrix) == 1:
                    density_matrix = list(density_matrix.values())[0] # Extract the density_matrix if we only have one
