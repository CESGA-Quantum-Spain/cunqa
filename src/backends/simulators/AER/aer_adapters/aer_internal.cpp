...
void NoiseModelDeleter::operator()(AER::Noise::NoiseModel* ptr) const {
    delete ptr;
}

NoiseModelPtr make_noise_model() {
    return NoiseModelPtr(new AER::Noise::NoiseModel());
}

NoiseModelPtr make_noise_model(const JSON& j) {
    return NoiseModelPtr(new AER::Noise::NoiseModel(j));
}

JSON run_circuit_with_controller(const JSON& aer_quantum_task, AER::Noise::NoiseModel& noise_model)
{
    AER::Circuit circuit(aer_quantum_task);
    std::vector<std::shared_ptr<AER::Circuit>> circuits;
    circuits.push_back(std::make_shared<AER::Circuit>(circuit));

    AER::Config aer_config(aer_quantum_task.at("config").get<JSON>());

    AER::Result result = controller_execute<AER::Controller>(circuits, noise_model, aer_config);
...
JSON simulate_standard(AerComputationAdapter& qc, const Backend* backend, AER::Noise::NoiseModel& noise_model){
    ...
    try {
        ...
        if (quantum_task.config.contains("seed")) {
            aer_quantum_task["config"]["seed_simulator"] = quantum_task.config.at("seed");
        }

        JSON result_json = aer_detail::run_circuit_with_controller(aer_quantum_task, noise_model);
...
// Cualquier otra función que necesite tipos completos de Aer
// (por ejemplo la llamada a controller_execute) también vive aquí
