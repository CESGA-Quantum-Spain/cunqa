...
std::string get_qc_run_command(const CunqaArgs& args, const std::string& mode)
{
    ...
    run_command =  "srun -n " + std::to_string(args.n_qpus) + " -c 1 --mem-per-cpu=1G --resv-ports=" + std::to_string(num_ports) + " --exclusive --task-epilog=$EPILOG_PATH setup_qpus " +  subcommand + " &\n";
