...
bool write_qc_run_command(std::ofstream& sbatchFile,const CunqaArgs& args)
{
    ...
    if (!args.gpu) {
        ...
        run_command +=  "srun --exclusive  -n 1 -c " + std::to_string(simulator_n_cores) + " --mem=" + std::to_string(simulator_memory) + "G setup_executor " + args.simulator + " " + std::to_string(args.n_qpus) + " " + std::to_string(args.n_comm_qubits) + "\n";
    } else {
#if !COMPILATION_FOR_GPU
        LOGGER_ERROR("CUNQA was not compiled with GPU support.");
        return false;
        ...
        // This is done to avoid run conditions in the IP publishing of the QPUs for the executor
        run_command += "sleep 1\n";
        run_command +=  "srun --exclusive -n 1 -c " + std::to_string(simulator_n_cores) + " --mem=" + std::to_string(simulator_memory) + "G --gres=gpu:1 setup_executor " + args.simulator + " " + std::to_string(args.n_qpus) + " " + std::to_string(args.n_comm_qubits) + "\n";
    }
