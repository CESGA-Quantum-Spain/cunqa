...
qjob = run(circ_upgrade, qpu, var_values={"x": np.pi, "y": 0, "z": 0}, shots=1024)
...
from cunqa.circuit.parameter import CunqaParameter, parameters
...
class TestVariables(unittest.TestCase):
    ...
    def test_non_assigned_variables_error(self):  
        ...
        # Create circuit with CunqaParameter parameters
        var_circuit = CunqaCircuit(3)
        var_circuit.rx(CunqaParameter("x"), 0)

        return self.assertRaises(SystemExit, self.qpus[-1].run, var_circuit)
    

    def test_variable_expressions(self):
        """Checks wether expressions constructed from CunqaParameter parameters, which are `sympy` objects, are allowed in CunqaCircuits and evaluate correctly."""
        x = parameters('x:4')
    ...
    def test_assign_works(self):
        ...
        circ_assign.rx(CunqaParameter("x"), 0)
        circ_assign.ry(CunqaParameter("y"), 1)
        circ_assign.rz(CunqaParameter("z"), 2)

        assign_values = {CunqaParameter("x"): 4, CunqaParameter("y"): 5.7, CunqaParameter("z"): 109.89}
    ...
    def test_variables_n_convert(self):
        ...
        CunqaParameter expressions <-> qiskit.ParameterExpressions"""

        # Translate minimal CunqaCircuit w Parameter(Expression)s to QuantumCircuit
        cunqac = CunqaCircuit(2)
        cunqac.rx(CunqaParameter("x")                                , 0) # x
        cunqac.rz(sp.sin(CunqaParameter("y")/2) + CunqaParameter("z") + 3.2, 1) # sin(y/2) + z +3.2
    ...
    def test_compatible_variables_n_transpile(self):
        ...
        x = parameters('x:2'); y = parameters('y:2')
    ...
    def test_upgrade_variable_parameters(self):
        """Checks wether upgrade_parameters behaves as expected when upgrading CunqaParameter Parameters. Depends on assign_parameters."""

        circ_upgrade = CunqaCircuit(3)

        circ_upgrade.rx(CunqaParameter("x"), 0)
        circ_upgrade.rx(CunqaParameter("y"), 1)
        circ_upgrade.rx(CunqaParameter("z"), 2)

        circ_upgrade.measure_all()

        results = []
        # First, assign values to Parameters
        circ_upgrade.assign_parameters({CunqaParameter("x"): np.pi, CunqaParameter("y"): 0, CunqaParameter("z"): 0})
        qjob = self.qpus[-2].run(circ_upgrade, shots=1024, transpile = False)
        results.append(qjob.result.counts)

        # Upgrade with dicts
        qjob.upgrade_parameters({CunqaParameter("x"): 0, CunqaParameter("y"): 0, CunqaParameter("z"): 0})
        results.append(qjob.result.counts)

        # Upgrade with a dict with only some of the Variables (previous values are preserved)
        qjob.upgrade_parameters({CunqaParameter("x"): 0, CunqaParameter("y"): np.pi})
        results.append(qjob.result.counts)
